...
- `run_after_01-decrypt-secrets.sh` - Auto-decrypts secrets on every apply if encrypted file is newer
...
6. **The `run_after` script runs on every apply** - `run_after_01-decrypt-secrets.sh` checks timestamps and only re-decrypts if the encrypted file is newer
